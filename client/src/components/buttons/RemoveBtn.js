  ...
  const [deleteCars] = useMutation(DELETE_CARS, {
    update(cache, { data: { deleteCars } }) {
      ...
      cache.writeQuery({
        query: GET_CARS,
        data: {
          cars: filter(cars, (car) => car.personId !== deleteCars.personId),
        },
      });
    },
  });
  ...
  const onClick = () => {
    ...
    if (props.id) {
      confirmation = window.confirm(
        'Would you like to delete this person and all their cars?'
      );
    ...
    if (props.carId) {
      confirmation = window.confirm('Would you like to delete this item?');
      if (confirmation && props.carId) {
        deleteCar({ variables: { id: carId } });
      }
    }
  };
  return <DeleteOutlined style={{ marginLeft: '16px' }} onClick={onClick} />;
};
